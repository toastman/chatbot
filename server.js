...
bot.configureIncomingWebhook({
    url: 'https://hooks.slack.com/services/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'
})

controller.setupWebserver(PORT, (err, webserver) => controller.createWebhookEndpoints(controller.webserver))
...
const getResponse = ({id, bot, message}) => {
  axios
    ...
    .catch((error) => bot.reply(message, `Something goes wrong. ${JSOn.stringify(error)}`))
}
